...
## Environment Check & AI Instructions
Before using these commands, verify if the tool is installed by running `agent-browser --version` via `exec`.
If the command is not found, ask the user for permission to install it globally via `npm install -g agent-browser`.
On Windows (PowerShell), if the global `agent-browser` command is not recognized due to PATH issues, use `npx agent-browser` instead.
